...
#Write the file name: France Group 1_BWS_CHEESE
...
d_1<-bws.dataset(
  ...
  model = "marginal",# the type of dataset created by the function
)
...
barplot(
  ...
  las = 1,
  error.bar = c("sd", "se", "ci"))

####Modelling approach####

####MNL###

#Case 1 BWS analysis on the basis of marginal model. 
...
# Data set for the marginal model
mr.data <- dfidx(data = DATA.full, 
                 idx = list(c("STR", "id"), "ALT"), 
                 choice = "RES")

#Model and output
mr.out <- mlogit(formula = mf, data = mr.data)
summary(mr.out)
...
sp.mr <- bws.sp(mr.out, base = "ITEM9")
sp.mr
...
rpl1 <- mlogit(
  ...
  data = mr.data, 
  rpar = c(ITEM1 = "n", ITEM2 = "n", ITEM3 = "n", ITEM4 = "n", ITEM5 = "n", ITEM6 = "n",
           ITEM7 = "n", ITEM8 = "n", ITEM10 = "n", ITEM11 = "n", ITEM12 = "n", ITEM13 = "n", ITEM14 = "n"),
...
data_mixl<- mlogit.data(data = DATA.full, choice = "RES", shape = "long",
                        alt.var = "ALT", chid.var = "STR", id.var = "id")
